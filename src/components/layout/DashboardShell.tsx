  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FC] font-sans text-slate-900 antialiased dark:bg-[#0B0C10] dark:text-slate-100">
      ...
      >
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between px-2">
            ...
            <button
              ...
              className="hidden h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 md:flex dark:hover:bg-slate-800"
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => setMobileMenuOpen(false)}
                  title={
                    sidebarCollapsed && !mobileMenuOpen ? item.label : undefined
                  }
        ...
        <div className="border-t border-slate-100 pt-4 dark:border-slate-800">
          <button
            ...
            className={`flex w-full cursor-pointer items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-slate-500 transition-all hover:bg-red-50 hover:text-red-600 dark:text-slate-400 dark:hover:bg-red-950/40 dark:hover:text-red-400 ${
              sidebarCollapsed && !mobileMenuOpen ? "justify-center px-0" : ""
            }`}
          >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Topbar Header */}
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-md sm:h-20 sm:px-8 dark:border-slate-800/80 dark:bg-[#121318]/90">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={() => router.back()}
              className="hidden cursor-pointer items-center gap-1.5 rounded-xl border border-slate-200/80 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-xs transition-all hover:bg-slate-50 sm:flex dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              ...
              {pathSegments.map((segment, index) => {
                ...
                return (
                  <React.Fragment key={path}>
                    ...
                    {isLast ? (
                      <span className="rounded-lg bg-blue-600 px-2 py-0.5 font-bold whitespace-nowrap text-white">
                        {formatSegment(segment)}
                      </span>
                    ) : (
                      <Link
                        href={path}
                        className="whitespace-nowrap text-slate-600 hover:text-blue-600 dark:text-slate-400"
                      >
                        {formatSegment(segment)}
                      </Link>
                    )}
          ...
          <div className="flex items-center gap-3">
            <button className="relative flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border border-slate-200/80 text-slate-600 transition-all hover:bg-slate-100 sm:h-10 sm:w-10 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800">
              <Bell className="h-4 w-4 sm:h-4.5 sm:w-4.5" />
              <span className="absolute top-2 right-2.5 h-2 w-2 rounded-full bg-blue-600"></span>
            </button>

            <div className="flex items-center gap-2 sm:gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-amber-500 text-sm font-bold text-white shadow-xs sm:h-10 sm:w-10 sm:text-base">
